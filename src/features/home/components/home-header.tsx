      ...
      <SiteShell>
        <div className="flex min-h-[84px] items-center justify-between gap-3 px-2 md:gap-6 md:px-4 xl:px-12">
          ...
          <nav className="hidden items-center gap-7 lg:flex">
            {navItems.map((item) => (
              <Link
                ...
                href={item.href}
                className="text-sm font-medium text-[#c9c2bc] transition hover:text-white"
              >
                {item.label}
              </Link>
            ))}
          ...
          <div className="hidden items-center gap-3 md:flex">
            ...
            <Link href="#demo" className={headerCtaClassName}>
              Thử đồ AI
            </Link>
          </div>

          <div className="flex items-center gap-1.5 lg:hidden">
            <Link href="#demo" className={`${mobileHeaderCtaClassName} max-[420px]:hidden`}>
